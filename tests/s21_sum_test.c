...
START_TEST(sum_matrix_1) {
  ...
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 2, &B);
  ck_assert_int_eq(s21_sum_matrix(&A, &B, (void *)0), ERROR_INIT);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(s21_sum_test_empty) {
  matrix_t A = {0}, B = {0}, result = {0};
  ck_assert_int_eq(ERROR_INIT, s21_sum_matrix(&A, &B, &result));
}
...
START_TEST(s21_sum_test_columns) {
  ...
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(1, 2, &B);
  ck_assert_int_eq(ERROR_CALC, s21_sum_matrix(&A, &B, &result));
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
